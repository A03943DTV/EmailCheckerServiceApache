...
public class RestService {
	...
	public Response excutePost(String targetURL, String urlParameters, String emailId) {
		...
		try {
			...
			int statusCode = connection.getResponseCode();
			if (statusCode != 200) {
				return new Response(FAILURE, null, new ErrorMessage("Invalid HTTP response status code " + statusCode + " from URL " + POST_URL));
			}
			...
			e.printStackTrace();
			return new Response(FAILURE, null, new ErrorMessage("Exception in connecting to the URL and getting response"));
		} finally {
			if (connection != null) {
				connection.disconnect();
			}
	...
	private Response verifyResponseContent(BufferedReader br, String emailId) throws IOException {
		...
		while (((readLine = br.readLine()) != null)) {
			if (readLine.contains(DIV_CLASS_EMAIL_RED)) {
				...
				return new Response(SUCCESS, new EmailChecker(emailId, isValid, getContent(readLine, DIV_CLASS_EMAIL_RED.length())), null);
			} else if (readLine.contains(DIV_CLASS_EMAIL)) {
				isDivElementPresent = true;
				isValid = false;
				return new Response(SUCCESS, new EmailChecker(emailId, isValid, getContent(readLine, DIV_CLASS_EMAIL.length())), null);
			}
		}
		if (!isDivElementPresent) {
			return new Response(FAILURE, null, new ErrorMessage("Response page doesn't contain required content"));
		}
		return null;
	}
